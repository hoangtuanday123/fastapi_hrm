...
from globalvariable import roleuser,is_admin,rolegroup,selectionItem,tablesession,messages,writerights,readrights,image_path_adminsession,fullname_adminsession,idaccountadminmanager,roleadmin,image_path_session
import pyotp
import pandas as pd
import pdfkit
import zipfile
...
@admin.get("/adminpage/roles/deleterole/{idrole}",tags=['user managerment'])
...
async def assignrole(request:Request,response:Response,idaccount,userrole,current_user: User = Depends(get_current_user_from_token)):
...
def info(idaccount,response:Response,current_user: User = Depends(get_current_user_from_token)):
    ...
    if temp[0]=="admin":
        ...
        if not found_avatar:
            found_avatar=file_path_default
